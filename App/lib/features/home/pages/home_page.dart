...
import '../../voice/pages/speech_page.dart';
import '../../payments/pages/pay_to_contacts_page.dart';
import '../../payments/pages/pay_to_phone_page.dart';
import '../../payments/pages/pay_to_upi_page.dart';
import '../../payments/pages/qr_scanner_page.dart';
import '../../transactions/pages/history_page.dart';
import '../../balance/pages/balance_page.dart';
import '../../payments/pages/request_money_page.dart';
import '../../payments/pages/my_qr_page.dart';
...
class _HomePageState extends ConsumerState<HomePage>
  ...
  // Local UI state only — the data below is sourced from Riverpod providers
  // and cached into the fields each build for the helper widgets to read.
  ...
  /// Pull the latest provider values into the cache fields the helper
  /// widgets read. Called at the top of [build]; safe because `build`
  /// re-runs whenever any watched provider changes.
  void _syncFromProviders() {
    ...
    _balance = balanceAsync.valueOrNull;
    _loadingBalance = balanceAsync.isLoading;

    final txAsync = ref.watch(transactionsProvider);
    _loadingTransactions = txAsync.isLoading;
    _recentTransactions = txAsync.valueOrNull?.take(4).toList() ?? const [];
  }
  ...
  Widget _activityRow(TransactionItem t) {
    final isSent = t.isSent;
    final name = t.counterpartyName;
    final amount = t.amount;
    final when = _relativeTime(t.timestamp);
    return Pressable(
      scale: 0.98,
      onTap: () => Navigator.push(context,
          MaterialPageRoute(builder: (_) => const HistoryPage())),

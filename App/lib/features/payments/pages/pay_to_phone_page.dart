...
class PayToPhonenumberBody extends ConsumerStatefulWidget {
  const PayToPhonenumberBody({super.key});

  @override
  ConsumerState<PayToPhonenumberBody> createState() =>
      PayToPhonenumberBodyState();
}

class PayToPhonenumberBodyState extends ConsumerState<PayToPhonenumberBody> {
  ...
  Future<void> _searchUser() async {
    ...
    final user = await ref.read(paymentRepositoryProvider).searchByPhone(phoneNumber);
    if (!mounted) return;
    setState(() {
      _loading = false;
      if (user != null) {
  ...
  Future<void> _sendMoney() async {
    ...
    final senderPhone = ref.read(sessionStoreProvider).phoneNumber ?? '';
    final receiverPhone = _controller.text.trim();
    final remark = _remarkController.text.trim();
    final result = await ref.read(paymentRepositoryProvider).sendByPhone(
    ...
    setState(() {
      _sending = false;
      _sendResult = result.success ? 'Payment successful!' : result.message;
    });
    if (result.success) {

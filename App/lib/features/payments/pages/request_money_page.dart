...
class _RequestMoneyPageState extends ConsumerState<RequestMoneyPage>
  ...
  Future<void> _loadRequests() async {
    ...
    try {
      final data =
          await ref.read(paymentRepositoryProvider).getMoneyRequests(phoneNumber);
      setState(() {
        _sentRequests =
  ...
  Future<void> _updateRequestStatus(int requestId, String status) async {
    final phoneNumber = ref.read(sessionStoreProvider).phoneNumber;
    final result = await ref.read(paymentRepositoryProvider).updateRequestStatus(
          requestId: requestId,
          status: status,
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          result.success ? 'Request $status successfully' : result.message,
        ),
      ),
    );
    if (result.success) _loadRequests();
  }
...
class RequestMoneyDialog extends ConsumerStatefulWidget {
  final VoidCallback onRequestSent;
  const RequestMoneyDialog({super.key, required this.onRequestSent});

  @override
  ConsumerState<RequestMoneyDialog> createState() =>
      _RequestMoneyDialogState();
}

class _RequestMoneyDialogState extends ConsumerState<RequestMoneyDialog> {
  ...
  Future<void> _sendRequest() async {
    ...
    final requesterPhone = ref.read(sessionStoreProvider).phoneNumber ?? '';

...
class _SpeechScreenState extends ConsumerState<SpeechScreen>
  ...
  Future<void> _loadUserPhone() async {
    _userPhone = ref.read(sessionStoreProvider).phoneNumber;
  }

  void _checkServerConnection() async {
    final isConnected =
        await ref.read(voiceRepositoryProvider).checkServerHealth();
    if (mounted) setState(() => _serverConnected = isConnected);
  }

  String? get _phone => _userPhone ?? ref.read(sessionStoreProvider).phoneNumber;
  ...
  Future<void> _processVoiceCommand(String text) async {
    ...
    try {
      final outcome = await ref
          ...
          .processCommand(text, userPhone: _phone ?? '');
      final data = {
        'amount': double.tryParse(outcome.amount ?? '') ?? outcome.amount,
      ...
      if (outcome.success) {
        ...
        } else if (outcome.isBalance) {
          await _checkAndShowBalance();
        } else if (outcome.isRequest) {
          setState(() => _isProcessing = false);
          _showRequestConfirmation(data);
        } else {
          setState(() {
            _isProcessing = false;
            _responseMessage = outcome.message;
            _isSuccess = true;
          });
        }
      } else {
  ...
  Future<void> _checkAndShowBalance() async {
    ...
    try {
      final balance =
          await ref.read(accountRepositoryProvider).getBalance(userPhone);
      setState(() => _isProcessing = false);
  ...
  Future<void> _executeMoneyTransfer(Map<String, dynamic> data) async {
    ...
    if (recipient != null && recipient.contains('@')) {
      result = await payments.sendByUpiId(
      ...
      result = await payments.sendByPhone(
        senderPhone: senderPhone,
  ...
  Future<void> _executeMoneyRequest(Map<String, dynamic> data) async {
    ...
    final result = await ref.read(paymentRepositoryProvider).createMoneyRequest(
